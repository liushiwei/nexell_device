				   ...
				   const u8 *replay_ctr)
{
...
#ifdef REALTEK_WIFI_VENDOR
	{
		...
		len += os_strlcpy(buf, "GTK_REKEY_OFFLOAD", sizeof(buf));
		len ++; /* NULL-termination */
		os_memcpy(buf+len, kek, NL80211_KEK_LEN);
		len += NL80211_KEK_LEN;
		os_memcpy(buf+len, kck, NL80211_KCK_LEN);
		len += NL80211_KCK_LEN;
		os_memcpy(buf+len, replay_ctr, NL80211_REPLAY_CTR_LEN);
		len += NL80211_REPLAY_CTR_LEN;

		wpa_hexdump(MSG_MSGDUMP, "GTK_REKEY_OFFLOAD", buf, len);

		wpa_driver_nl80211_driver_cmd(priv, "GTK_REKEY_OFFLOAD", buf, len);
	}
#endif /* REALTEK_WIFI_VENDOR */
