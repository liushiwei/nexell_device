...
function update_2ndboot()
{
    if [ ${UPDATE_2NDBOOT} == "true" ] || [ ${UPDATE_ALL} == "true" ]; then
		...
		if [ ${BOOT_DEVICE_TYPE} == "nand" ]; then
			...
			${TOP}/linux/platform/${CHIP_NAME}/tools/bin/BOOT_BINGEN -c nxp4330 -t 2ndboot -o ${secondboot_out_file} -i ${secondboot_file} -n ${nsih_file} ${option_p}
		fi
        sync
        sleep 1
        echo "call fastboot"
...
function update_root()
{
    if [ ${UPDATE_ALL} == "true" ] || [ ${UPDATE_ROOT} == "true" ]; then
        ...
        cp -a ${TOP}/out/target/product/s5p4418_msd8000b/root ${RESULT_DIR}/
